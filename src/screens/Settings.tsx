...
const REMINDER_TEXT: Record<ReminderStatus, string> = {
  ...
  off: 'Примерно раз в сутки приложение напомнит, если за день не учтено ни минуты, — даже закрытое.',
  'not-installed':
...
const CHECK_TEXT: Record<RemindResult | 'denied' | 'unsupported', string> = {
  ...
  nothing: 'Напоминать не о чем — за сегодня время уже учтено. Пришло пустое уведомление, чтобы было видно, что они доходят.',
        ...
        <>
          ...
          <div className="row row--wrap">
            {(status === 'off' || status === 'not-installed') && (
              ...
              >
                Напоминать о незаполненном дне
              </button>
            )}
            {status === 'on' && (
              <button
